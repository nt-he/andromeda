...
session = scoped_session(db.session_fact)
...
@app.route("/load")
def load_miis():
        miis = [{"loc": f, "username": session.query(models.CachedUser).get(f.split(".")[0]).username + "#" + session.query(models.CachedUser).get(f.split(".")[0]).discriminator} for f in os.listdir("miis") if os.path.isfile(os.path.join("miis", f))]
        if request.args:
